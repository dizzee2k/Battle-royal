...
const BATTLE_TURN_COIN_REWARD = 5; // New: Coins for surviving each turn
...
function attemptCatch(userId, ballType) {
    ...
    if (Math.random() < catchChance) {
        ...
        user.inventory[ballType.toLowerCase()]--; // Decrement even on failure
        userData.set(userId, user);
        saveGameState();
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isButton()) {
        ...
        if (action === 'target') {
            ...
            // Revalidate possible targets after status effects
            const possibleTargets = Array.from(battle.players.entries())
                .filter(([id]) => id !== userId)
                .map(([id, data]) => ({ id, pokemon: data.pokemon }));
            ...
            // Check if the selected target is still valid
    ...
    try {
        ...
        if (commandName === 'startbattle') {
            ...
            for (const [userId, data] of battle.players.entries()) {
                ...
                // Award initial participation coins to non-bot players
                if (!data.isBot) {
                    const user = userData.get(userId);
                    user.coins += BATTLE_COIN_REWARD;
                    userData.set(userId, user);
                    await channel.send(`<@${userId}> earned ${BATTLE_COIN_REWARD} coins for joining the battle!`);
                }
            }
            embed.addFields(fields);
            saveGameState();
