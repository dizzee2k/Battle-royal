...
async function botPlayTurn(botId, botData, channel) {
    ...
    // Check status effects before allowing the bot to act
    ...
    // Get possible targets (exclude the bot itself)
    ...
    if (difficulty === 'easy') {
        // Easy mode: Random move and target
        ...
        // Normal mode: 50% chance to act randomly, 50% chance to prioritize super effective moves
        if (Math.random() < 0.5) {
            move = botPokemon.moves[Math.floor(Math.random() * botPokemon.moves.length)];
            targetId = possibleTargets[Math.floor(Math.random() * possibleTargets.length)].id;
        } else {
            ...
            // If no super effective move is found, pick randomly
        ...
        // Hard mode: Always choose the most effective move against the weakest target
        let bestMove = null;
        let bestTargetId = null;
...
async function announceBattleUpdate(channel) {
    try {
        ...
        // Build fields with Pokémon names, labeling opponents and bots
        const fields = [];
...
client.once('ready', async () => {
    ...
    await channel.send('Pokémon Battle Royale Bot is online! Use `/joinbattle player` to join, `/joinbattle bot <quantity> <difficulty>` to add bots, then `/startbattle` to begin.');
});
        ...
        if (commandName === 'joinbattle') {
            ...
            if (options.getSubcommand() === 'player') {
                ...
                for (let i = 0; i < botsToAdd; i++) {
                    const botId = `bot_${Date.now()}_${i}`; // Unique ID for each bot
                    const pokemon = getRandomPokemon();
