...
function attemptCatch(userId, ballType, fromBattle = false) {
    ...
    const caughtPokemonName = wildPokemon.name; // Capture name before clearing
    const catchRate = shopItems[ballType.toLowerCase()].catchRate * (2 - hpRatio) + 0.1; // Base 10% chance even at full HP
    const catchChance = Math.min(1, catchRate * (wildPokemon.shiny ? 0.5 : 1)); // Shiny Pokémon are harder to catch
    ...
    if (Math.random() < catchChance) {
        ...
        if (user.party.length < MAX_PARTY_SIZE) {
            user.party.push({ ...wildPokemon }); // Create a copy to avoid reference issues
        } else {
        ...
        if (wildBattle && fromBattle) {
            wildBattle = null; // End the battle only if catching from within
        }
...
client.on('interactionCreate', async interaction => {
    if (interaction.isAutocomplete()) {
        ...
        if (focusedOption.name === 'pokemon' || focusedOption.name === 'target') {
            ...
            await interaction.respond(pokemonList.slice(0, 25)); // Limit to 25 options for autocomplete
        }
        return;
    }
    ...
    if (interaction.isButton()) {
        ...
        if (interaction.customId.startsWith('catch_modal_')) {
            const ballType = interaction.fields.getTextInputValue('ball_type').toLowerCase();
            const result = attemptCatch(userId, ballType, wildBattle);
            if (result.success) {
                await interaction.reply({ content: `<@${userId}> caught the wild ${result.pokemonName}! Earned ${CATCH_COIN_REWARD} coins and ${CATCH_XP_REWARD} XP!`, ephemeral: false });
            } else {
                await interaction.reply({ content: `<@${userId}> failed to catch the wild ${result.pokemonName}!`, ephemeral: false });
            }
            await announceBattleUpdate(channel);
            return;
        }
        ...
        if (action === 'view_inventory') {
            const user = userData.get(userId) || { inventory: {}, party: [], pc: [], coins: 0 };
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor('#00ff00')
                .addFields(
                    { name: 'Items', value: inventoryList || 'Empty!', inline: true },
                    { name: 'Party', value: user.party.map(p => `${p.name} (Level ${p.level}${p.shiny ? ' ★' : ''})`).join(', ') || 'Empty!', inline: true },
                    { name: 'PC', value: user.pc.map(p => `${p.name} (Level ${p.level}${p.shiny ? ' ★' : ''})`).join(', ') || 'Empty!', inline: true },
        ...
        if (action === 'fight') {
            ...
            const moveSelect = new ModalBuilder()
                .setCustomId(`fight_move_${userId}`)
                .setTitle('Select Move to Use');
            ...
            const targetInput = new TextInputBuilder()
                ...
                .setLabel('Choose target (ID or leave blank for wild)')
                .setStyle(TextInputStyle.Short)
                .setRequired(false);

            const firstActionRow = new ActionRowBuilder().addComponents(moveInput);
            const secondActionRow = new ActionRowBuilder().addComponents(targetInput);
            moveSelect.addComponents(firstActionRow, secondActionRow);

            await interaction.showModal(moveSelect);
            return;
        }

        if (interaction.customId.startsWith('fight_move_')) {
            const moveName = interaction.fields.getTextInputValue('move_name');
            const targetId = interaction.fields.getTextInputValue('target_id') || (wildBattle ? wildBattle.botId : battle.turnOrder[(battle.currentTurn + 1) % battle.turnOrder.length]);
            const user = userData.get(userId);
            let playerData = battle.players.get(userId) || (wildBattle && wildBattle.playerId === userId ? { pokemon: wildBattle.playerPokemon, ownerId: userId, isBot: false } : null);

            if (!playerData || !playerData.pokemon.moves.includes(moveName)) {
                await interaction.reply({ content: 'Invalid move or Pokémon!', ephemeral: true });
                return;
            }

            let targetData;
            if (wildBattle) {
                targetData = { pokemon: wildPokemon, ownerId: wildBattle.botId, isBot: true };
            } else {
                targetData = battle.players.get(targetId);
            }
            ...
            const targetLabel = wildBattle ? 'wild Pokémon' : (targetData.isBot ? `${targetData.pokemon.name} (bot)` : `${targetData.pokemon.name} (opponent)`);
            await channel.send(`${playerData.pokemon.name} used ${moveName} on ${targetLabel} for ${damage} damage!`);
            ...
            if (!wildBattle && checkBattleEnd(channel)) {
                battle.players.clear();
                battle.turnOrder = [];
                battle.turnMessage = null;
                saveGameState();
        ...
        if (action === 'flee') {
            ...
            if (wildBattle) {
                await channel.send(`${playerData.pokemon.name} fled from the wild ${wildPokemon.name}!`);
                wildBattle = null;
            } else {
                await channel.send(`${playerData.pokemon.name} fled from the battle!`);
        ...
        if (action === 'item') {
            ...
            const targetInput = new TextInputBuilder()
                ...
                .setLabel('Choose target Pokémon')
                .setStyle(TextInputStyle.Short)
                .setRequired(true);

            const firstActionRow = new ActionRowBuilder().addComponents(itemInput);
            const secondActionRow = new ActionRowBuilder().addComponents(targetInput);
            itemSelect.addComponents(firstActionRow, secondActionRow);

            await interaction.showModal(itemSelect);
            return;
        }

        if (interaction.customId.startsWith('item_use_')) {
            const itemName = interaction.fields.getTextInputValue('item_name').toLowerCase();
            const targetName = interaction.fields.getTextInputValue('target_name');
            const user = userData.get(userId);
            const targetPokemon = user.party.find(p => p.name === targetName);

            if (!targetPokemon || !useItem(userId, itemName, targetPokemon)) {
                await interaction.reply({ content: `Failed to use ${itemName.replace('ball', ' Ball').replace('powder', ' Powder')}!`, ephemeral: true });
                return;
            }

            await interaction.reply({ content: `<@${userId}> used ${itemName.replace('ball', ' Ball').replace('powder', ' Powder')} on ${targetPokemon.name}!`, ephemeral: false });
            await announceBattleUpdate(channel);
            return;
        }
    ...
    try {
        if (commandName === 'startadventure') {
            ...
            const result = attemptCatch(userId, ball, wildBattle);
            if (result.success) {
                await interaction.reply({ content: `<@${userId}> caught the wild ${result.pokemonName}! Earned ${CATCH_COIN_REWARD} coins and ${CATCH_XP_REWARD} XP!`, ephemeral: false });
            } else {
                await interaction.reply({ content: `<@${userId}> failed to catch the wild ${result.pokemonName}!`, ephemeral: false });
            ...
            if (wildPokemon) {
                await interaction.reply({ content: `A wild Pokémon is already present: ${wildPokemon.name} (Level ${wildPokemon.level}${wildPokemon.shiny ? ' ★' : ''})! Use /fightwild to battle or /catch to try catching it.`, ephemeral: true });
                return;
            }

            if (wildBattle) {
                await interaction.reply({ content: 'A wild Pokémon battle is already in progress! Finish the battle first.', ephemeral: true });
                return;
            }
            ...
            lastSpawnTime = Date.now();
            const shinyText = wildPokemon.shiny ? ' ★ (Shiny)' : '';
            await interaction.reply({ content: `You searched and found a wild ${wildPokemon.name} (Level ${wildPokemon.level}${shinyText})! Use /fightwild to battle or /catch to try catching it.`, ephemeral: false });
            await announceBattleUpdate(channel);
            ...
            if (!user || !user.inventory[item.toLowerCase()] || user.inventory[item.toLowerCase()] <= 0) {
            ...
            if (useItem(userId, item, targetPokemon)) {
                await interaction.reply({ content: `<@${userId}> healed ${targetPokemon.name} with ${item.replace('ball', ' Ball').replace('powder', ' Powder')}!`, ephemeral: false });
            } else {
                await interaction.reply({ content: `Failed to heal ${targetPokemon.name} with ${item.replace('ball', ' Ball').replace('powder', ' Powder')}!`, ephemeral: true });
            }
            await announceBattleUpdate(channel);
            ...
            if (!user) {
                await interaction.reply({ content: 'You haven\'t started your adventure yet!', ephemeral: true });
                return;
            }

            let pokemonArray = location === 'party' ? user.party : user.pc;
            const pokemonIndex = pokemonArray.findIndex(p => p.name === pokemonName);
            if (pokemonIndex === -1) {
                await interaction.reply({ content: 'Pokémon not found in your selected location!', ephemeral: true });
                return;
            }
            ...
            userData.set(userId, user);
            saveGameState();
            await interaction.reply({ content: `Released ${pokemonName} from your ${location}!`, ephemeral: false });
            await announceBattleUpdate(channel);
        } else if (commandName === 'trade') {
            await interaction.reply({ content: 'Trading feature is under development! Stay tuned for updates.', ephemeral: true });
        }
    } catch (error) {
