...
const moveData = {
    ...
    'Poison Powder': { baseDamage: 10, accuracy: 75, type: 'Poison', status: true },
...
async function announceBattleUpdate(channel) {
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            .setThumbnail('https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/25.png') // Example Pokémon thumbnail (Pikachu)
            .addFields(
                ...Array.from(battle.players.entries()).map(([userId, data]) => ({
                    name: `<@${userId}>`,
                    value: `**${data.pokemon.name}: HP <span style="color:red">**${data.pokemon.currentHp}/${data.pokemon.hp}**</span>**`
                }))
            )
