...
function applyStatusEffect(defender, move, channel) {
    ...
    channel.send(`<@${defender.ownerId}>'s ${defender.name} is now ${defender.status}!`);
    return true;
}
...
function handleStatusEffects(playerData, channel) {
    ...
    if (pokemon.status === 'paralyzed' && Math.random() < 0.25) {
        channel.send(`<@${playerData.ownerId}>'s ${pokemon.name} is paralyzed and can't move!`);
        canAct = false;
    } else if (pokemon.status === 'asleep') {
        if (pokemon.statusTurns > 0) {
            channel.send(`<@${playerData.ownerId}>'s ${pokemon.name} is asleep and can't move!`);
            pokemon.statusTurns--;
            canAct = false;
        } else {
            channel.send(`<@${playerData.ownerId}>'s ${pokemon.name} woke up!`);
            pokemon.status = 'none';
        }
        ...
        channel.send(`<@${playerData.ownerId}>'s ${pokemon.name} took ${poisonDamage} damage from poison!`);
        ...
        channel.send(`<@${playerData.ownerId}>'s ${pokemon.name} took ${burnDamage} damage from its burn!`);
    }

    if (pokemon.currentHp === 0) {
        channel.send(`<@${playerData.ownerId}>'s ${pokemon.name} has fainted due to status effects! <@${playerData.ownerId}> is eliminated!`);
...
async function announceBattleUpdate(channel) {
    try {
        ...
        // Fetch usernames and build fields
        const fields = [];
        for (const [userId, data] of battle.players.entries()) {
            ...
            fields.push({
                name: `@${username}: ${data.pokemon.name}${statusText}`,
