...
function levelUpPokemon(pokemon, channel) {
...
async function botPlayTurn(botId, botData, channel) {
    ...
    if (checkBattleEnd()) {
        ...
        if (!winnerData.isBot) {
            ...
            await channel.send(`<@${winnerId}> earned ${BATTLE_ROYALE_WIN_COIN_REWARD} coins and ${BATTLE_ROYALE_WIN_XP_REWARD} XP for winning the Battle Royale!`);
        }
...
client.once('ready', async () => {
    ...
    await channel.send('Pokémon Battle Royale Bot is online! Use `/startadventure` to begin your journey, then use `/joinbattle`, `/startbattle`, `/shop`, `/bal`, and `/catch` to play.');
});
...
client.on('interactionCreate', async interaction => {
    ...
    // Check if user has started their adventure (except for /startadventure)
    ...
    try {
        ...
        if (commandName === 'attack') {
            ...
            levelUpPokemon(attackerData.pokemon, channel);
            ...
            // Award coins for participating in battle
            const user = userData.get(userId);
