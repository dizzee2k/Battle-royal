...
const INVENTORY_LIMIT = 6; // Max Pokémon in inventory
...
// Expanded Pokémon data with fixed base stats (including Gen 1-3 starters)
const pokemonData = [
...
let userData = new Map(); // Stores user inventory and PC
...
function getRandomPokemon(excludeStarters = false) {
    ...
    return { ...pokemon, currentHp: pokemon.hp, status: 'none', statusTurns: 0 };
}
...
async function spawnWildPokemon(channel) {
    const wildPokemon = getRandomPokemon(true);
    const catchChance = Math.random() * 100; // Base 50% chance to catch, adjustable
    channel.send(`A wild ${wildPokemon.name} appeared! Use /catch ${wildPokemon.name} to try catching it! (Catch chance: ${Math.floor(catchChance)}%)`);
    return { pokemon: wildPokemon, catchChance };
}
...
client.once('ready', async () => {
    ...
    await channel.send('Pokémon Battle Royale Bot is online! Use `/startjourney` to begin your journey, `/joinbattle player` to join battles, `/joinbattle bot <quantity> <difficulty>` to add bots, `/startbattle` to begin a battle, or `/catch` to catch wild Pokémon!');
});
    ...
    try {
        if (commandName === 'joinbattle') {
            ...
            if (options.getSubcommand() === 'player') {
                ...
                const pokemon = { ...userDataEntry.inventory[0], currentHp: userDataEntry.inventory[0].hp }; // Use first Pokémon from inventory
        ...
        if (commandName === 'startjourney') {
            ...
            userData.set(userId, {
                inventory: [{ ...starter, currentHp: starter.hp, status: 'none', statusTurns: 0 }],
                pc: []
            });
            saveGameState();
            await interaction.reply({ content: `You started your journey with ${starterName}! Use /catch to catch wild Pokémon and /inventory to view your team.`, ephemeral: true });
        }

        if (commandName === 'catch') {
            const targetPokemonName = options.getString('pokemon');
            const wildPokemon = pokemonData.find(p => p.name === targetPokemonName);
            if (!wildPokemon) {
            ...
            const userDataEntry = userData.get(userId) || { inventory: [], pc: [] };
            const catchChance = Math.random() * 100; // Simulate catch chance (e.g., 50% base)
            if (catchChance < 50) { // Adjust catch rate as needed
                await interaction.reply({ content: `Failed to catch ${targetPokemonName}! Try again or use a better strategy.`, ephemeral: true });
                return;
            }

            const caughtPokemon = { ...wildPokemon, currentHp: wildPokemon.hp, status: 'none', statusTurns: 0 };
            if (userDataEntry.inventory.length < INVENTORY_LIMIT) {
                userDataEntry.inventory.push(caughtPokemon);
                await interaction.reply({ content: `Caught ${targetPokemonName}! It has been added to your inventory.`, ephemeral: true });
            } else {
                userDataEntry.pc.push(caughtPokemon);
                await interaction.reply({ content: `Caught ${targetPokemonName}! Your inventory is full, so it was sent to your PC. Use /pc to manage your storage.`, ephemeral: true });
            }
        ...
        if (commandName === 'pc') {
            ...
            if (action === 'withdraw' && pokemonName) {
                ...
                userData.set(userId, userDataEntry);
                saveGameState();
                await interaction.reply({ content: `Withdrew ${pokemonName} from PC to your inventory!`, ephemeral: true });
                ...
                userData.set(userId, userDataEntry);
                saveGameState();
                await interaction.reply({ content: `Deposited ${pokemonName} to your PC!`, ephemeral: true });
            } else {
...
setInterval(async () => {
    if (!battle.active || !battle.battleStartTime) {
        const channel = await client.channels.fetch(BATTLE_CHANNEL_ID);
        if (Math.random() < 0.1) { // 10% chance to spawn a wild Pokémon every 5 seconds
