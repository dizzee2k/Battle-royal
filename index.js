...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isButton()) {
        ...
        if (action === 'target') {
            ...
            if (possibleTargets.length === 0) {
                // If the target is invalid, prompt the player to select a new move or target
                const moves = playerData.pokemon.moves;
                const moveButtons = moves.map((move, index) => {
                    const moveInfo = moveData[move];
                    return new ButtonBuilder()
                ...
                await interaction.update({ content: 'Target is no longer in battle or invalid! Please select a new move:', components: rows });
                return;
            }
