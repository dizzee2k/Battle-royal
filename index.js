...
function attemptCatch(userId, ballType) {
    if (!wildPokemon) {
        console.log(`No wild Pokémon to catch for user ${userId}`);
        return false;
    }
    ...
    // Log the ball type and inventory for debugging
    console.log(`Attempting to catch with ballType: ${ballType}, Inventory:`, user.inventory);

    // Ensure ballType matches the inventory key (lowercase)
    ...
    const catchRate = shopItems[ballType.toLowerCase()].catchRate;
    const hpRatio = wildPokemon.currentHp / wildPokemon.hp;
    const catchChance = catchRate * (1 - hpRatio) * (wildPokemon.shiny ? 1.5 : 1);

    if (Math.random() < catchChance) {
        user.inventory[ballType.toLowerCase()]--; // Decrement the ball count
        ...
        wildPokemon = null; // Clear the wild Pokémon after a successful catch
        userData.set(userId, user);
        saveGameState();
        console.log(`User ${userId} successfully caught ${wildPokemon.name} with ${ballType}`);
        return true;
    } else {
        user.inventory[ballType.toLowerCase()]--; // Decrement even on failure (as in most Pokémon games)
        userData.set(userId, user);
        saveGameState();
...
// Modified handleStatusEffects to only check the current player's Pokémon and avoid eliminating others mid-turn
...
async function botPlayTurn(botId, botData, channel) {
    ...
    // Apply status effects to the target after the move
...
async function announceBattleUpdate(channel) {
    try {
        ...
        // If a wild Pokémon is present, add it to the embed
        if (wildPokemon) {
            const shinyText = wildPokemon.shiny ? ' ★ (Shiny!)' : '';
            embed.addFields({
                name: 'Wild Pokémon',
                value: `A wild ${wildPokemon.name} (Level ${wildPokemon.level}${shinyText}) has appeared! Use /catch <ball> to try catching it!`
            });
        }
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isButton()) {
        ...
        if (action === 'target') {
            ...
            // Log the state of battle.players for debugging
            console.log(`Target selection: userId=${userId}, move=${move}, targetId=${targetId}, battle.players=`, Array.from(battle.players.keys()));
            ...
            if (!battle.players.has(targetId)) {
                console.log(`Target ${targetId} is no longer in the battle. Current players:`, Array.from(battle.players.keys()));
                const moves = playerData.pokemon.moves;
                const moveButtons = moves.map((move, index) => {
                    const moveInfo = moveData[move];
                    return new ButtonBuilder()
    ...
    try {
        ...
        if (commandName === 'catch') {
            ...
            if (!wildPokemon) {
                await interaction.reply({ content: 'There is no wild Pokémon to catch right now!', ephemeral: true });
                return;
            }

            if ((user.inventory[ball] || 0) <= 0) {
                await interaction.reply({ content: `You don't have any ${ball.replace('ball', ' Ball')}s in your inventory!`, ephemeral: true });
                return;
            }
            ...
            if (attemptCatch(userId, ball)) {
                ...
                await interaction.reply({ content: `Failed to catch ${wildPokemon.name}! The Pokémon broke free! You have ${user.inventory[ball] || 0} ${ball.replace('ball', ' Ball')}s left.`, ephemeral: true });
            }
        }
    } catch (error) {
