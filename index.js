...
function handleStatusEffects(playerData, channel) {
    ...
    if (pokemon.currentHp === 0) {
        channel.send(`${pokemon.name} has fainted due to status effects! Player is eliminated!`);
...
async function announceBattleUpdate(channel) {
    try {
        ...
        // Build fields with Pokémon names, labeling opponents
        const fields = [];
        for (const [userId, data] of battle.players.entries()) {
            const isCurrentPlayer = userId === currentPlayerId;
            const pokemonLabel = isCurrentPlayer ? data.pokemon.name : `${data.pokemon.name} (opponent)`;
...
client.on('interactionCreate', async interaction => {
    if (interaction.isAutocomplete()) {
        ...
        if (focusedOption.name === 'target') {
            const targets = Array.from(battle.players.entries()).map(([id, data]) => {
                ...
                const targetLabel = isCurrentPlayer ? `${data.pokemon.name} (user)` : `${data.pokemon.name} (opponent)`;
                return {
                    name: targetLabel,
                    value: id
                };
    ...
    try {
        ...
        if (commandName === 'attack') {
            ...
            if (damage > 0) {
                const targetLabel = targetId === userId ? `${targetData.pokemon.name} (user)` : `${targetData.pokemon.name} (opponent)`;
            ...
            if (checkBattleEnd()) {
                ...
                await channel.send(`The battle is over! Winner: ${winnerData.pokemon.name}!`);
