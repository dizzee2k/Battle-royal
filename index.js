...
function attemptCatch(userId, ballType, fromBattle = false) {
    ...
    if (Math.random() < catchChance) {
        ...
        wildPokemon = null;
        if (wildBattle) wildBattle = null; // End the battle if catching from within
...
async function botPlayTurn(botId, botData, channel, isWild = false) {
    ...
    const difficulty = botData.difficulty || 'normal'; // Wild Pokémon use "normal" difficulty
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isButton()) {
        ...
        if (action === 'useitem') {
            ...
            if (item === 'pokeball' || item === 'greatball' || item === 'ultraball') {
                ...
                if (attemptCatch(userId, item, true)) {
                    await channel.send(`<@${userId}> caught the wild ${wildPokemon.name}! Earned ${CATCH_COIN_REWARD} coins and ${CATCH_XP_REWARD} XP!`);
                    await interaction.update({ content: `Successfully caught ${wildPokemon.name}!`, components: [] });
                    await announceBattleUpdate(channel);
                } else {
                    await channel.send(`<@${userId}> failed to catch the wild ${wildPokemon.name}!`);
                    await interaction.update({ content: `Failed to catch ${wildPokemon.name}!`, components: [] });
                    wildBattle.currentTurn++;
                    await announceBattleUpdate(channel);
                }
                return;
        ...
        if (action === 'target') {
            ...
            if (damage > 0) {
                const targetLabel = wildBattle ? `Wild ${targetData.pokemon.name}` :
                  (targetData.isBot ? `${targetData.pokemon.name} (bot)` : `${targetData.pokemon.name} (opponent)`);
