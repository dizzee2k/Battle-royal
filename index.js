...
const { Client, IntentsBitField, SlashCommandBuilder, EmbedBuilder, ButtonBuilder, ButtonStyle, ActionRowBuilder } = require('discord.js');
const fs = require('fs');
...
function attemptCatch(userId, ballType, fromBattle = false) {
    ...
    // Adjusted catch rate: 2 - hpRatio makes low HP Pokémon easier to catch, plus a small base chance
    ...
    if (Math.random() < catchChance) {
        ...
        saveGameState();
        console.log(`User ${userId} successfully caught ${wildPokemon ? wildPokemon.name : 'unknown Pokémon'} with ${ballType}`);
        return true;
    } else {
        user.inventory[ballType.toLowerCase()]--;
        userData.set(userId, user);
...
client.once('ready', async () => {
    ...
    await channel.send('Pokémon Battle Royale Bot is online! Use `/startadventure` to begin your journey, then use `/joinbattle`, `/startbattle`, `/shop`, `/bal`, `/inventory`, `/catch`, `/fightwild`, `/searchwild`, and `/use` to play.');
});
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isButton()) {
        ...
        if (action === 'item') {
            const user = userData.get(userId);
            const items = Object.keys(shopItems).filter(item => shopItems[item].healAmount || shopItems[item].cure || (wildBattle && (item === 'pokeball' || item === 'greatball' || item === 'ultraball')));
            const itemButtons = items.map((item, index) => {
                return new ButtonBuilder()
                    .setCustomId(`useitem_${userId}_${item}_${index}`)
                    .setLabel(`${item.replace('ball', ' Ball').replace('powder', ' Powder')}: ${user.inventory[item] || 0}`)
                    .setStyle(ButtonStyle.Secondary);
            });
        ...
        if (action === 'move') {
            ...
            if (wildBattle) {
                possibleTargets = [{ id: 'wild', pokemon: wildPokemon, isBot: true }];
            } else {
                possibleTargets = Array.from(battle.players.entries())
                    .filter(([id]) => id !== userId)
                    .map(([id, data]) => ({ id, pokemon: data.pokemon, isBot: data.isBot }));
            ...
            console.log(`Player ${userId} selecting target for move ${move}. Possible targets:`, possibleTargets.map(t => t.id));

            if (possibleTargets.length === 0) {
                console.log(`No targets available for player ${userId}`);
            ...
            const targetButtons = possibleTargets.map((target, index) => {
                const targetLabel = wildBattle ? `Wild ${target.pokemon.name}` : (target.isBot ? `${target.pokemon.name} (bot)` : `${target.pokemon.name} (opponent)`);
                return new ButtonBuilder()
        ...
        if (action === 'useitem') {
            ...
            if (item === 'pokeball' || item === 'greatball' || item === 'ultraball') {
                if (!wildBattle) {
                    await interaction.update({ content: 'You can only use Poké Balls during wild Pokémon battles!', components: [] });
                    return;
                }
                if (attemptCatch(userId, item, true)) {
                    await channel.send(`<@${userId}> caught the wild ${wildPokemon ? wildPokemon.name : 'unknown Pokémon'}! Earned ${CATCH_COIN_REWARD} coins and ${CATCH_XP_REWARD} XP!`);
                    await interaction.update({ content: `Successfully caught ${wildPokemon ? wildPokemon.name : 'unknown Pokémon'}!`, components: [] });
                    await announceBattleUpdate(channel);
                } else {
                    await channel.send(`<@${userId}> failed to catch the wild ${wildPokemon ? wildPokemon.name : 'unknown Pokémon'}!`);
                    await interaction.update({ content: `Failed to catch ${wildPokemon ? wildPokemon.name : 'unknown Pokémon'}!`, components: [] });
                    wildBattle.currentTurn++;
                    await announceBattleUpdate(channel);
                }
                return;
        ...
        if (action === 'target') {
            ...
            console.log(`Player ${userId} executing move ${move} on target ${targetId}. Possible targets:`, possibleTargets.map(t => t.id));
            ...
            if (!targetData) {
                console.log(`Target ${targetId} not found for player ${userId}`);
                if (wildBattle) wildBattle.currentTurn++;
                else battle.currentTurn++;
                await announceBattleUpdate(channel);
                await interaction.deferUpdate();
    ...
    try {
        if (commandName === 'startadventure') {
            ...
            if (attemptCatch(userId, ball)) {
                await interaction.reply({ content: `<@${userId}> caught the wild ${wildPokemon ? wildPokemon.name : 'unknown Pokémon'}! Earned ${CATCH_COIN_REWARD} coins and ${CATCH_XP_REWARD} XP!`, ephemeral: false });
                await announceBattleUpdate(channel);
            } else {
                await interaction.reply({ content: `<@${userId}> failed to catch the wild ${wildPokemon ? wildPokemon.name : 'unknown Pokémon'}!`, ephemeral: false });
                await announceBattleUpdate(channel);
            }
            ...
            if (wildPokemon) {
                await interaction.reply({ content: `A wild Pokémon is already present: ${wildPokemon.name} (Level ${wildPokemon.level})! Use /fightwild to battle or /catch to try catching it.`, ephemeral: true });
                return;
            }
