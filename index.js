...
const { Client, IntentsBitField, SlashCommandBuilder, EmbedBuilder, ApplicationCommandOptionType } = require('discord.js');
const fs = require('fs');
        ...
        new SlashCommandBuilder()
            ...
            .addStringOption(option =>
                option.setName('target')
                    .setDescription('The target to attack (mention user or Pokémon name)')
                    .setRequired(true)
            ),
        new SlashCommandBuilder()
...
client.on('interactionCreate', async interaction => {
    ...
    try {
        ...
        if (commandName === 'startbattle') {
            ...
            await interaction.reply({ content: 'Battle starting now!', ephemeral: true });
            await channel.send('Battle starting now with current players!');
            await announceBattleUpdate(channel);

            // Announce whose turn it is
            const firstPlayerId = battle.turnOrder[0];
            await channel.send(`It's <@${firstPlayerId}>'s turn! Use /attack to make your move.`);
        ...
        if (commandName === 'attack') {
            ...
            if (!attackerData.pokemon.moves.includes(move)) {
                await interaction.reply({
                    content: `Invalid move! Your available moves are: ${attackerData.pokemon.moves.join(', ')}`,
                    ephemeral: true
                });
                return;
            }
            ...
            if (targetInput.startsWith('<@') && targetInput.endsWith('>')) {
                ...
                if (!battle.players.has(targetId) || targetId === userId) {
                    await interaction.reply({ content: 'Invalid target! Please select a valid opponent.', ephemeral: true });
                    return;
                }
            } else {
                ...
                if (!targetId || targetId === userId) {
                    await interaction.reply({ content: 'Invalid target! Please select a valid Pokémon or opponent.', ephemeral: true });
                    return;
                }
            }
            ...
            if (battle.players.size > 0) {
                ...
                await channel.send(`It's <@${nextPlayerId}>'s turn! Use /attack to make your move.`);
            }
...
client.on('interactionCreate', async interaction => {
    ...
    if (commandName === 'attack' && battle.active && battle.players.has(user.id)) {
        const playerData = battle.players.get(user.id);
        const focusedValue = interaction.options.getFocused();
        const filteredMoves = playerData.pokemon.moves.filter(move => move.toLowerCase().includes(focusedValue.toLowerCase()));
        await interaction.respond(
            filteredMoves.map(move => ({ name: move, value: move }))
        );
    }
});
