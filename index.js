...
const SHINY_CHANCE = 1 / 8192; // Classic shiny rate
const EXP_PER_LEVEL = 100; // Base EXP needed to level up

// Add new constants for experience calculation
const BASE_EXP_GAIN = 50; // Base experience gained from battles
const WINNER_EXP_MULTIPLIER = 1.5; // Winners get 50% more experience
const LEVEL_SCALING_FACTOR = 1.2; // Each level requires 20% more exp than the previous

// Update the battle-related constants
const BATTLE_CHECK_INTERVAL = 5000; // Check battle status every 5 seconds
const SPAWN_CHECK_INTERVAL = 5000; // Check spawn every 5 seconds
...
const SPAWN_INTERVAL = Math.floor(Math.random() * (300000 - 180000) + 180000); // Random 3-5 minutes
const BASE_CATCH_RATE = 0.4; // 40% base catch rate
const STARTING_MONEY = 100; // Starting currency for new players

// Pokémon type effectiveness (multipliers)
const typeEffectiveness = {
    // ... [Previous type effectiveness data remains the same]
};

// Expanded Pokémon data with evolutions and levels
const pokemonData = [
    ...
    // ... [Add all Gen 1-3 Pokemon with evolution data]
];

// Expanded move data with weather effects
const moveData = {
    // ... [Previous move data remains the same with added weather bonuses]
    'Solar Beam': {
        baseDamage: 30,
        accuracy: 80,
        type: 'Grass',
    ...
    // ... [More moves with weather effects]
};

// Modify the game state to include wild Pokémon spawns
let gameState = {
    wildPokemon: null,
    lastSpawnTime: Date.now()
};

// Modify the battle object
let battle = {
    active: false,
    players: new Map(),
    currentTurn: 0,
    ...
    weather: 'Clear', // New weather system
    lastTurnTime: null,
    turnTimeout: TURN_TIMEOUT
};

let userData = new Map(); // Store user's Pokemon collection
let wildPokemon = null; // For wild encounters
...
// Enhanced random Pokemon generator with shiny chance
...
// New function to handle evolution
...
// Enhanced damage calculation with weather effects
...
// Modify the levelUpPokemon function to be more comprehensive
function levelUpPokemon(pokemon) {
    pokemon.level++;

    // Calculate stat increases (5% per level)
    const statGain = 1.05;
    ...
    pokemon.currentHp = pokemon.hp; // Heal Pokemon on level up

    // Return the stat changes for notification
    return {
...
// Add new function to calculate required exp for next level
function getRequiredExp(level) {
    return Math.floor(EXP_PER_LEVEL * Math.pow(LEVEL_SCALING_FACTOR, level - 1));
}

// Add new function to award experience
...
// Modify the battle end section to include experience gains
...
client.on('interactionCreate', async interaction => {
    ...
    if (commandName === 'start') {
        ...
        const helpEmbed = new EmbedBuilder()
            ...
            .addFields(
                { name: '/start', value: 'Choose your starter Pokémon to begin your journey.', inline: true },
                { name: '/join_battle', value: 'Join the current battle royale.', inline: true },
                { name: '/attack', value: 'Attack another player with a move.', inline: true },
                { name: '/check_stats', value: 'View your Pokémon\'s stats.', inline: true },
                { name: '/inventory', value: 'Check your items and Pokémon collection.', inline: true },
                { name: '/weather', value: 'Check the current battle weather.', inline: true },
                { name: '/shop', value: 'View available Poké Balls for purchase.', inline: true },
                { name: '/buy', value: 'Purchase Poké Balls with coins.', inline: true },
                { name: '/catch', value: 'Attempt to catch a wild Pokémon.', inline: true },
                { name: '/bal', value: 'Check your balance and inventory.', inline: true },
        ...
        // ... [Weather command implementation if defined elsewhere]
